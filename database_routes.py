...
def setup_routes(socketio):
    def fetch_and_store_data():
        try:
            # Beispiel für das Abrufen der Daten. Ersetze dies durch tatsächliches Abrufen der Daten.
            solar_data = SolarPosition.get_position()
            heliostat_data = HeliostatState.get_direction()
    ...
    def handle_save_request():
        if fetch_and_store_data():
            load_positions()
            emit('data_saved', {'message': 'Solar- und Heliostat-Daten erfolgreich zusammen gespeichert'})
        else:
    ...
    def load_positions():
        ...
        positions_data = [
            {
                'id': pos.id, 'solar_azimuth': pos.solar_azimuth, 'solar_elevation': pos.solar_elevation,
                'heliostat_azimuth': pos.heliostat_azimuth, 'heliostat_elevation': pos.heliostat_elevation
    ...
    def delete_selected_positions(data):
        ...
        for id in selected_ids:
            ...
            if position:
                db.session.delete(position)
        db.session.commit()
        emit('selected_positions_deleted', {'selected_ids': selected_ids}, broadcast=True)
